...
export const thokk = (splits: number = 2, separator: string = '_'): string => {
  if (!splits || splits < 1) return ''
  let artifacts: string[] = []
  for (let split = 1; split <= splits - 1; split++) {
    let adjectiveIndex = Math.floor(Math.random() * ADJECTIVES.length)
    artifacts.push(ADJECTIVES[adjectiveIndex])
  }
